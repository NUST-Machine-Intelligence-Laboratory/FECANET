...
class DatasetPASCAL(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # EXP_2_1 history_mask的初始化
        spatial_size = 100  # 数据集初始化设置的，根据参数logit_mask
        if self.history_mask_list[idx] is None:
            history_mask = torch.zeros(2, spatial_size, spatial_size).fill_(0.0)
        else:
            # 贪心算法选取
            if random.random() > self.prob:  # HyperParameter_setting
        ...
        batch = {'query_img': query_img,
                 ...
                 'query_name': self.img_metadata[idx][0],
